...
**狀態:** ✅ 已完成(`eef8e89`;CI 6/6 jobs 全綠,0 annotations)。
...
- [x] **Step 1: test_smoke.py(完整程式碼;無 mosqito 依賴,CI 可跑)**
...
- [x] **Step 2: `.gitignore` 追加(同 P1 注意:只揀本任務的 hunk)**
...
- [x] **Step 1: 從 Rust 端取實測值(R3-P2 已放好工具)**
...
- [x] **Step 2: 把印出的值填入 test_smoke.py 的 `N_HASH`/`TIME_HASH`**

**反作假條款:必須是上一步實際印出的值。**填入後刪除兩行「凍結步驟(Task 3)…」註解。

- [x] **Step 3: 驗證 green(跨語言逐位契約成立的時刻)**
...
- [x] **Step 1: 更新 requirements.lock(venv 多了 maturin/pytest)**
...
- [x] **Step 2: tools/bench_binding.py(spec §13 指標:binding 開銷 ≤ +2%)**
...
- [x] **Step 4: ci.yml 追加 py job(`jobs:` 層級)**
...
- [x] **Step 6: 請使用者確認 GitHub Actions**
...
- CI:`test`/`ffi`/`py` 三個 job 雙平台共 6 個 matrix jobs 全綠(`eef8e89`),0 annotations。
- parity 實測:18 passed,耗時 66.10 s;最大絕對偏差 `1.2513812208680974e-09`(`step_60_80/zwtv/N_specific`),容差 rtol `1e-6`/atol `1e-9`。
- bitwise 契約:`N_HASH=0x44e6822074554786`,`TIME_HASH=0xf076bcb342595537`(凍結自 `dump_py_bitwise_contract_hashes`)。
- binding 開銷:Python best-of-20 44.0 ms vs Cargo Criterion AVX2 中位數 47.697 ms(同日同機),差 -7.8%,達成不高於 +2% 的門檻。
- wheel:`wheel-windows-latest` / `wheel-ubuntu-latest` artifacts 均由綠燈 upload job 產出;另以 Windows clean venv 安裝 wheel 後 smoke 6/6 通過。
- 核心隔離:`git diff --stat e96dffa..HEAD -- iso532/src/` 為空;Rust 全 suite、fmt、clippy 與 12 個 golden hash 全綠。
- 偏差:
  - `iso532-py/Cargo.lock` 納入版本控制,沿用 `iso532-ffi` 的可重現交付策略。
  - 當前 Clippy 對 Python tuple 回傳型別要求具名 type aliases;純型別重構,無行為差異。
  - mosqito 1.2.1 以包含終點的 `linspace` 產生 duration-dependent time axis,與 ISO 固定 2 ms grid 不同;parity 比較 N/N_specific/Bark,Python smoke 與 parity 另以 bitwise/exact assertion 看守 Rust time axis。
  - 初次 CI 全綠但有 Node 20 deprecation annotations;已升級至 Node 24 actions,並為 `setup-msvc-dev@v4` 明確指定 `arch: x64`,最終 `eef8e89` 全綠且無 annotations。
