...
pub unsafe fn nl_loudness_avx2(core: &[f64], n_time: usize) -> Vec<f64> {
    ...
    #[inline]
    unsafe fn load4(core: &[f64], n_time: usize, band: usize, t: usize) -> __m256d {
        _mm256_set_pd(
    ...
    unsafe fn process4(core: &[f64], out: &mut [f64], n_time: usize, band: usize, b: [f64; 6]) {
        ...
        let last = n_time - 1;
        let last_row = load4(core, n_time, band, last);
        let last_delta = _mm256_mul_pd(_mm256_sub_pd(_mm256_setzero_pd(), last_row), inv_iter);
        let mut prev_uo =
            _mm256_fmadd_pd(last_delta, _mm256_set1_pd((NL_ITER - 1) as f64), last_row);
        let mut prev_u2 = _mm256_setzero_pd();

        for col in 0..(n_time * NL_ITER) {
            let t = col / NL_ITER;
            let k = col % NL_ITER;
            let row = load4(core, n_time, band, t);
            let next = if t + 1 < n_time {
                load4(core, n_time, band, t + 1)
            } else {
                _mm256_setzero_pd()
            };
    ...
    for band in (0..20).step_by(4) {
        process4(core, &mut out, n_time, band, b);
    }

    let mut tail_core = vec![0.0; 21 * n_time];
    tail_core[..n_time].copy_from_slice(&core[20 * n_time..21 * n_time]);
    let tail = nl_loudness_scalar(&tail_core, n_time);
    out[20 * n_time..21 * n_time].copy_from_slice(&tail[..n_time]);
